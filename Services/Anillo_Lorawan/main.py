...
def main():
    try:
        db_devnet_data = get_devnet_data(table_name="anillo_lorawan")
        
        interface_status = get_status_prtg(db_devnet_data)
        ...
        if result is True:
            datetime_register(system_name="anillo_lorawan", status="OK")
        else:
            datetime_register(system_name="anillo_lorawan", status="ERROR")
