...
def update_devnet_data(data):
    """
    Actualiza el estado de las interfaces en la base de datos `dcs.anillo-tetra` utilizando los datos proporcionados.

    Esta función toma una lista de diccionarios que contienen el estado de la interfaz (`status`) y el 
    identificador de PRTG (`objid`). Actualiza la columna `status` en la tabla `anillo-tetra` de la base de datos 
    ...
    query = """
    UPDATE devnet.`anillo_lorawan` SET 
        status = %s
    WHERE id_prtg = %s
    """

    # Construimos los tuplas que contienen el `status` y el `id_prtg` para cada entrada
    data_tuple = [(interface["status"], interface["objid"]) for interface in data]

    try:
        logging.info("Actualizando datos en la base de datos...")
